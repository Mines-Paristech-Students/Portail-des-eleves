...
class LoanableTestCase(BaseLibraryTestCase):
    ...
    def test_if_not_library_admin_then_cannot_create_loanable(self):
        for user in ALL_USERS_EXCEPT_LIBRARY_BD_TEK:
            ...
            data = {
                "name": "La Piche",
                "description": "Manuel de la Piche",
    ...
    def test_if_library_admin_then_can_create_loanable(self):
        ...
        self.assertEqual(
            Loanable.objects.get(name=data["name"]).description, data["description"]
        )
        self.assertEqual(
            Loanable.objects.get(name=data["name"]).comment, data["comment"]
        )
        self.assertIn(
            Loanable.objects.get(name=data["name"]),
            Library.objects.get(pk=data["library"]).loanables.all(),
        )
