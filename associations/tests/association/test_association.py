...
from associations.tests.association.base_test_association import BaseAssociationTestCase, ALL_USERS_EXCEPT_GLOBAL_ADMIN


class AssociationTestCase(BaseAssociationTestCase):
    EXPECTED_FIELDS = {'id', 'name', 'logo', 'is_hidden', 'rank', 'marketplace', 'library', 'pages', 'my_role'}
    ...
    def test_if_not_global_admin_then_cannot_create(self):
        for user in ALL_USERS_EXCEPT_GLOBAL_ADMIN:
            ...
            self.assertFalse(Association.objects.filter(pk=self.create_association_data['id']).exists())
