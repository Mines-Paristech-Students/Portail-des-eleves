...
class BalanceTestCase(BaseMarketPlaceTestCase):
    ...
    def test_if_logged_in_then_can_retrieve_balance(self):
        ...
        self.assertEqual(Decimal(content[0]['balance']),
                         compute_balance(User.objects.get(pk='17simple'), Marketplace.objects.get(pk='biero')))
    ...
    def test_if_not_marketplace_admin_then_cannot_retrieve_balance_of_another_user(self):
    ...
    def test_if_logged_in_then_can_retrieve_balances(self):
        ...
        for market_balance in content:
            self.assertEqual(Decimal(market_balance['balance']),
    ...
    def test_if_library_administrator_then_can_retrieve_all_users_balances(self):
        ...
        for market_balance in content:
            ...
            self.assertEqual(Decimal(market_balance['balance']),
                             compute_balance(User.objects.get(pk=user_id),
                                             Marketplace.objects.get(pk=market_balance['marketplace'])))
    ...
    def test_balance_is_correctly_computed(self):
        ...
        def check_funding(biero_funding):
            ...
            biero_content = json.loads(self.get(f'/associations/marketplace/biero/balance/{consumer.id}/').content)
        ...
        self.post('/associations/funding/', data={'user': consumer.id, 'value': 10, 'marketplace': 'biero'})
        self.post('/associations/funding/', data={'user': consumer.id, 'value': 10, 'marketplace': 'biero'})
        check_funding(20)
        ...
        # Buy things.
        self.login(consumer.id)
        self.post('/associations/transactions/', data={'buyer': consumer.id, 'product': 3, 'quantity': 2})
        check_funding(0)

        # Buy and cancel.
        self.post('/associations/transactions/', data={'buyer': consumer.id, 'product': 3, 'quantity': 1})
        self.patch(f'/associations/transactions/{Transaction.objects.last().id}/', {'status': 'CANCELLED'})
        check_funding(0)

        # Buy and have one administrator reject the transaction.
        self.post('/associations/transactions/', data={'buyer': consumer.id, 'product': 3, 'quantity': 1})
        self.login(market_biero.id)
        self.patch(f'/associations/transactions/{Transaction.objects.last().id}/', {'status': 'REJECTED'})
        check_funding(0)
        ...
        self.login(market_biero.id)
        self.patch(f'/associations/transactions/{Transaction.objects.last().id}/', {'status': 'ACCEPTED'})
        check_funding(-5)

        self.login(market_biero.id)
        self.patch(f'/associations/transactions/{Transaction.objects.last().id}/', {'status': 'DELIVERED'})
        check_funding(-5)

        self.login(market_biero.id)
        self.patch(f'/associations/transactions/{Transaction.objects.last().id}/', {'status': 'REFUNDED'})
        check_funding(0)
