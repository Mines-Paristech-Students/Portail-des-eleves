...
class BalanceTestCase(BaseMarketPlaceTestCase):
    ...
    def test_if_logged_in_then_can_retrieve_balances(self):
        self.login('17simple')
        res = self.get('/marketplace/balance/')
        self.assertStatusCode(res, 200)
    ...
    def test_balance_is_correctly_computed(self):
        ...
        self.post('/funding/', data={'user': consumer.id, 'value': 10, 'marketplace': 'biero'})
        self.post('/funding/', data={'user': consumer.id, 'value': 10, 'marketplace': 'biero'})
        check_funding(20)
        ...
        self.patch(f'/funding/{funding_id}/', data={'status': 'REFUNDED'})
        check_funding(10)

        # Buy things.
        self.login(consumer.id)
        self.post('/transactions/', data={'buyer': consumer.id, 'product': 3, 'quantity': 2})
        check_funding(0)

        # Buy and cancel.
        self.post('/transactions/', data={'buyer': consumer.id, 'product': 3, 'quantity': 1})
        self.patch(f'/transactions/{Transaction.objects.last().id}/', {'status': 'CANCELLED'})
        check_funding(0)

        # Buy and have one administrator reject the transaction.
        self.post('/transactions/', data={'buyer': consumer.id, 'product': 3, 'quantity': 1})
        self.login(market_biero.id)
        self.patch(f'/transactions/{Transaction.objects.last().id}/', {'status': 'REJECTED'})
        check_funding(0)
        ...
        self.login(market_biero.id)
        self.patch(f'/transactions/{Transaction.objects.last().id}/', {'status': 'ACCEPTED'})
        check_funding(-5)

        self.login(market_biero.id)
        self.patch(f'/transactions/{Transaction.objects.last().id}/', {'status': 'DELIVERED'})
        check_funding(-5)

        self.login(market_biero.id)
        self.patch(f'/transactions/{Transaction.objects.last().id}/', {'status': 'REFUNDED'})
        check_funding(0)
