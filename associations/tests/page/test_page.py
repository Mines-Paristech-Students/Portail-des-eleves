...
class PageAssociationTestCase(BasePageTestCase):
    ...
    def test_if_not_page_admin_then_cannot_create_page(self):
        for user in ALL_USERS_EXCEPT_PAGE_BIERO:
            ...
            self.assertRaises(ObjectDoesNotExist, Page.objects.get, title=self.news_page_data['title'])
    ...
    def test_if_not_page_admin_then_cannot_update_page(self):
        for user in ALL_USERS_EXCEPT_PAGE_BIERO:
            ...
            res = self.update(self.valid_update_page_data['pk'], data=self.valid_update_page_data, association_id='biero')
            self.assertStatusCode(res, 403)
            self.assertEqual(Page.objects.get(pk=self.valid_update_page_data['pk']).title, page_before.title)
    ...
    def test_if_page_admin_then_cannot_update_association_field(self):
        self.login('17page_biero')
        res = self.update(self.invalid_update_page_data['pk'],
                          data=self.invalid_update_page_data,
                          association_id='biero')
        self.assertStatusCode(res, 200)
        self.assertNotEqual(Page.objects.get(pk=self.invalid_update_page_data['pk']).association_id,
