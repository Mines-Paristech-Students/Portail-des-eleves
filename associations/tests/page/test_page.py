...
class PageAssociationTestCase(BasePageTestCase):
    ...
    def test_if_not_page_admin_then_cannot_create_page(self):
        for user in ALL_USERS_EXCEPT_PAGE_BIERO:
            ...
            self.assertRaises(
                ObjectDoesNotExist, Page.objects.get, title=self.page["title"]
            )
    ...
    disallowed_update_page = {"pk": 0, "association": "pdm"}
    ...
    def test_if_not_page_admin_then_cannot_update_page(self):
        for user in ALL_USERS_EXCEPT_PAGE_BIERO:
            ...
            res = self.update(
                self.update_page["pk"], data=self.update_page, association_id="biero"
            )
            self.assertStatusCode(res, 403)
            self.assertEqual(
                Page.objects.get(pk=self.update_page["pk"]).title, page_before.title
            )
    ...
    def test_if_page_admin_then_cannot_update_association(self):
        self.login("17page_biero")
        res = self.update(
            self.disallowed_update_page["pk"],
            data=self.disallowed_update_page,
            association_id="biero",
        )
        self.assertStatusCode(res, 200)
        self.assertNotEqual(
            Page.objects.get(pk=self.disallowed_update_page["pk"]).association_id,
