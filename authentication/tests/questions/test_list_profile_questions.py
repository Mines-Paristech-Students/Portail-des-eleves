...
class TestListProfileQuestions(WeakAuthenticationBaseTestCase):
    ...
    def test_if_logged_in_then_can_list(self):
        for user in self.ALL_USERS:
            ...
            self.assertEqual(
                len(ProfileQuestion.objects.all()), len(res.data["questions"])
            )

            for question in res.data["questions"]:
                q = ProfileAnswer.objects.filter(
                    question=question["id"], user="17simple"
                )
