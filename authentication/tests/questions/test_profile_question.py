...
class TestProfileQuestion(BaseTestCase):
    ...
    def test_if_logged_in_then_can_list(self):
        for user in self.ALL_USERS:
            ...
            self.assertSetEqual(set(q.id for q in ProfileQuestion.objects.all()),
    ...
    def test_if_global_admin_then_can_create(self):
        ...
        self.assertEqual(ProfileQuestion.objects.last().text, self.profile_question_create_data['text'])
    ...
    def test_if_not_global_admin_then_cannot_update(self):
        ...
        self.assertStatusCode(res, 403)
        self.assertEqual(profile_question_before.text, ProfileQuestion.objects.get(pk=1).text)
    ...
    def test_if_global_admin_then_can_update(self):
        ...
        res = self.update(pk=1, data=self.profile_question_create_data)
        self.assertStatusCode(res, 200)
        self.assertEqual(self.profile_question_update_data['text'], ProfileQuestion.objects.get(pk=1).text)
