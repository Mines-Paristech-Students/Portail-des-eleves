...
class TestProfileAnswer(BaseTestCase):
    ...
    def test_if_logged_in_then_can_retrieve(self):
        for user in self.ALL_USERS:
            ...
            self.assertSetEqual({'id', 'text', 'question', 'user'}, set(res.data.keys()))
    ...
    def test_if_logged_in_then_can_list(self):
        for user in self.ALL_USERS:
            ...
            self.assertListEqual([q.id for q in ProfileAnswer.objects.all()],
                                 [q['id'] for q in res.data])
            self.assertListEqual([q.text for q in ProfileAnswer.objects.all()],
                                 [q['text'] for q in res.data])
            self.assertListEqual([q.question.id for q in ProfileAnswer.objects.all()],
                                 [q['question'] for q in res.data])
            self.assertListEqual([q.user.id for q in ProfileAnswer.objects.all()],
    ...
    profile_answer_17admin_create_data = {'text': 'La bière', 'question': 1, 'user': '17admin'}
    profile_answer_17admin_invalid_create_data = {'text': '0', 'question': 2, 'user': '17admin'}
    profile_answer_17simple_create_data = {'text': '42', 'question': 2, 'user': '17simple'}

    def test_can_create_for_own_profile(self):
        for user, data in (('17admin', self.profile_answer_17admin_create_data),
    ...
    def test_cannot_create_for_other_profile(self):
        for user, data in (('17simple', self.profile_answer_17admin_create_data),
    ...
    def test_can_update_own_answer(self):
        ...
        self.assertEqual(profile_answer.text, self.profile_answer_17simple_update_data['text'])

        self.login('17admin')
        res = self.update(pk=2, data=self.profile_answer_17admin_update_data)
        self.assertStatusCode(res, 200)
        profile_answer = ProfileAnswer.objects.get(pk=2)
        self.assertEqual(profile_answer.text, self.profile_answer_17admin_update_data['text'])
    ...
    def test_if_not_admin_then_cannot_update_answer_of_other_profile(self):
        ...
        self.assertStatusCode(res, 403)
        profile_answer = ProfileAnswer.objects.get(pk=2)
        self.assertNotEqual(profile_answer.text, self.profile_answer_17admin_update_data['text'])
    ...
    def test_cannot_update_user(self):
        ...
        self.assertStatusCode(res, 200)
        profile_answer = ProfileAnswer.objects.get(pk=2)
        self.assertNotEqual(profile_answer.user.id, self.profile_answer_17admin_invalid_update_data['user'])
    ...
    def test_cannot_update_question(self):
        ...
        self.assertStatusCode(res, 200)
        profile_answer = ProfileAnswer.objects.get(pk=2)
        self.assertNotEqual(profile_answer.question.id, self.profile_answer_17admin_invalid_update_data_2['question'])
