...
export const SidebarDateSelector = ({ association, setParams }) => {
  ...
  const availableYears =
    data && data.min && data.max
      ? Array.from(
          {
            length: data.max.getFullYear() + 1 - data.min.getFullYear(),
          },
          (_, k) => (k + (data.min || new Date()).getFullYear()).toString()
        )
      : [];
  ...
  return status === "loading" ? (
    ...
    <>
      <SidebarSection retractable title="Année" retractedByDefault={false}>
      ...
      <SidebarSection retractable title="Mois" retractedByDefault>
        {MONTHS.map((month) => (
          <CheckboxField
            label={month}
