...
def serializeRepartition(n, user):
    ...
    return {"id": n.id, "progress": progress, "title": n.title, "voeux": voeux, "promotion": n.promotion,
            "status": n.status, "equirepartition": n.equirepartition,
            "propositions": [serializeProposition(p) for p in queryset], "resultat": resultat}
...
class RepartitionsViewSet(views.APIView):
    ...
    def get(self, request, rid=None, **kwargs):
        ...
        if not user.is_admin:
            queryset = queryset.filter(
                status__gt=0)  # si l'utilisateur n'est pas admin, on prend que les repartitions commencées
        queryset = queryset.filter()
        if rid is not None:
    ...
    def put(self, request, rid=None, **kwargs):
        ...
        if not True:  # TODO canCreateNewRepartition():
            return JsonResponse(
                {"status": "error", "message": "You're not allowed to create a new repartition campaign"}, status="401")
        if not user.is_admin and "promotion" in body:
            return JsonResponse(
                {"status": "error", "message": "You're not allowed to set promotion in the new repartition campaign"},
                status="400")
        ...
        n.equirepartition = body["equirepartition"] if (
                    "equirepartition" in body and body["equirepartition"] in [True, False]) else True
        n.save()
    ...
    def patch(self, request, rid=None, **kwargs):
        ...
        if not True:  # TODO canCreateNewRepartition():
            return JsonResponse({"status": "error", "message": "You're not allowed to edit a repartition campaign"},
                                status="401")
        if not user.is_admin and "promotion" in body:
            return JsonResponse(
                {"status": "error", "message": "You're not allowed to set promotion in a repartition campaign"},
                status="400")
        ...
        try:
            ...
            for i, lv in enumerate(vs_voeux_unpack):
                for j in range(n_users):
                    a[i, j] = lv.index(
                        proposition_par_j[j])  # poids de l'assignation de i à la tache du tableau des taches j
