...
class TagRelatedToCase(TagsBaseTestCase):
    ...
    def test_related_to_filter(self):
        ...
        self.assertSetEqual(set(x["id"] for x in res.data["results"]), {1, 2, 3, 4})

        res = self.get("/tags/tags/?related_to=media")
        self.assertStatusCode(res, 200)
        self.assertSetEqual(set(x["id"] for x in res.data["results"]), {1, 3})

        res = self.get("/tags/tags/?related_to=product")
        self.assertStatusCode(res, 200)
        self.assertSetEqual(set(x["id"] for x in res.data["results"]), {2, 3})

        res = self.get("/tags/tags/?related_to=media,product")
        self.assertStatusCode(res, 200)
        self.assertSetEqual(set(x["id"] for x in res.data["results"]), {1, 2, 3})
