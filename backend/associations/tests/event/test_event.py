...
class EventsTestCase(BaseEventsTestCase):
    ...
    def test_if_not_event_admin_then_cannot_create_event(self):
        for user in ALL_USERS_EXCEPT_EVENTS_BIERO:
            ...
            self.assertRaises(
                ObjectDoesNotExist, Event.objects.get, name=self.event["name"]
            )

    def test_if_event_admin_then_can_create_event(self):
        self.login("17event_biero")  # Event administrator.
        res = self.create(association_id="biero", data=self.event)
    ...
    def test_cannot_update_association_field(self):
        ...
        self.assertStatusCode(res, 200)
        self.assertEqual(
            events_before.association.pk, Event.objects.get(pk=0).association.pk
        )
    ...
    def test_can_join_and_leave_event(self):
        for user in ALL_USERS:
            ...
            self.assertStatusCode(res, 200)
            self.assertTrue(
                User.objects.get(pk=user) in Event.objects.get(pk=1).participants.all(),
