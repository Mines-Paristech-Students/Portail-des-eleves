...
def get_widgets(request):
    ...
    for w in widgets:
        try:
            ...
            returned_dict["widgets"].append({"name": w.name, "displayed": displayed})
    ...
    for ass in associations:
        try:
            ...
            returned_dict["associations"].append(
                {"name": ass.id, "displayed_name": ass.name, "subscribed": subscribed}
            )
...
def set_subscriptions(request):
    ...
    for ass in associations:
        if not isinstance(ass, dict) or "name" not in ass or "subscribed" not in ass:
            raise ValidationError(
                "Association %s must be a dict with keys 'name' and 'subscribed' " % ass
            )
    ...
    for widg in widgets:
        if not isinstance(widg, dict) or "name" not in widg or "displayed" not in widg:
            raise ValidationError(
                "Widget %s must be a dict with keys 'name' and 'displayed'" % widg
            )
